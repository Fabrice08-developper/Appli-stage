<?php session_start(); 
  ...
  $fr= "SELECT NOM_CLIENT, NOM_DEMANDE FROM service, demander, client WHERE service.ID_TECH = :id_tech AND service.ID_SERVICE = demander.ID_SERVICE AND demander.ID = client.ID";
          ...
          <?php if (count($dem) === 0): ?>
            <div class="card">
              <div class="card-body">
                ...
                <p class="card-text">Vous n'avez actuellement aucune intervention assignée. Veuillezvérifier ultérieurement pour les mises à jour.</p>
              </div>
            </div>
          <?php else: ?>
          <?php for($i = 0; $i < count($dem); $i++): ?>
            ...
            <div class="col-md-6 mb-3">
                ...
                <div class="card-body">
                    ...
                    <p class="card-text">Client : <?= $dem[$i]['NOM_CLIENT'] ?><br>Statut : <span class="badge bg-success">_</span></p>
                    <button class="btn btn-outline-success btn-sm">accepter</button>
                    <button class="btn btn-outline-danger btn-sm">decliner</button>
                </div>
                </div>
            </div>
          <?php endfor; ?>
